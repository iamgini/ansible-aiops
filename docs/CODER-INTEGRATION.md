...
│    ↓                                     │
│ 4. AI review (if ai_review_enabled)     │
│    ↓                                     │
│ 5. Verify playbook generation           │
│    ↓                                     │
...
2. **MCP Query** (same as original workflow)
   ...
   - If MCP is unavailable and `mcp_api_fallback=true` (the default), automatically falls back to the AAP REST API
...
git push origin aiops/disk_alert-143022
```

**Note:** Generated playbooks are pushed to a review branch (`aiops/<event_type>-<HHMMSS>`), not `main`. If `ai_review_enabled=true`, a second AI pass reviews the generated playbook and pushes the reviewed version to `<branch>_review`.
